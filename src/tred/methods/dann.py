...
from tred.utils import GRL, BalancedDataloader, CombinedDataloader, metric_wrapper
...
class DANN(BaseMethod):
    ...
    def __init__(
        ...
        self.D_optim = Adam(self.D.parameters(), lr=self.lr / self.lambda_adapt)
    ...
    def fit(
        ...
        dataloader_C = BalancedDataloader(
            ...
            datasets={n: d for n, d in train_L.items() if n != target_name},
        ...
        for epoch in range(self.max_epochs):
            train_loss = self._train_epoch(dataloader_C, dataloader_D)
    ...
    def _train_epoch(
        self, dataloader_C: BalancedDataloader, dataloader_D: CombinedDataloader
    ) -> float:
        """
        ...
        for _ in range(self.batches_per_epoch):
            ...
            logits = self.C(self.E(batch["X"]))
            loss_class = self.BCE_loss(logits, batch["y"].unsqueeze(1).float())
            epoch_loss += loss_class.item()

            # Train domain discriminator
            batch = self._move_to_device(next(dataloader_D))
            logits = self.D(GRL.apply(self.E(batch["X"])))
            loss_domain = self.CE_loss(logits, batch["d"])
